...
<div>
  <h2>Authserver</h2>
  <p>Authserver is in charge of authentication. It also provides an endpoint to
    check if the user is authorised or not</p>
  <div>
    <label>Username</label><input bind:value={credentials.username} type="text"><br>
    <label>Password</label><input bind:value={credentials.password} type="text"><br>

    <button on:click={registerHandler}>Register</button>
    <button on:click={loginHandler}>Login</button>
    <button on:click={checkAuthHandler}>
...
<div>
  ...
  <div>
    <input bind:value={springPingContent} type="text">
    <button on:click={springPingHandler}>Send</button>
  </div>
</div>
